...
public class Task04 {
    public static void main(String[] args) {
        ...
        String regex4 = "(^\\d+$)|(^\\d(\\D)\\d{3}\\3\\d{1,3}.*)"; // регулярка для проверки формата числа правильная версия
        //регулярка разделена на 3 группы 1-я гр (^\d+$) - ищет целое число без разделителей; 2-я гр (^\d(\D)\d{3}\3\d{1,3}.*) - второе выражение полностью
        // 3-я гр ((\D)) - не число, поэтому ссылаемся на 3-ю группу при повторею ссылка производится без знака $!!!! (\\3)!!!
//String regex4 = "((\\d+)|(\\d/\\d{3}/\\d{1,3}/?.*)|(\\d\\*\\d{3}\\*\\d{1,3}\\*?.*)|(\\d-\\d{3}-\\d{1,3}-?.*)|" +
//                "(\\d_\\d{3}_\\d{1,3}_?.*)|(\\d\\+\\d{3}\\+\\d{1,3}\\+?.*)|(\\d\\.\\d{3}\\.\\d{1,3}\\.?.*)|(\\d\\s\\d{3}\\s\\d{1,3}\\s?.*)|" +
//                "(\\d,\\d{3},\\d{1,3},?.*))"; //регулярка для проверки формата числа длинная версия
        ...
        if(n4 == 0) {//если нажата кнопка №1 - "Ввести свое форматированое число"
            ...
            System.out.println(" <= Соответствие нужному формату (NNN,NNN,NNN) => " + mach);// вывод в консоль проверки формата числа
